...
def _push_tree(owner: str, repo: str, files: dict[str, str], message: str) -> None:
    ...
    def _post_with_retry(url: str, json_payload: dict, label: str) -> dict:
        """POST with up to 3 retries on 409, with 3/6/9 s back-off."""
        for attempt in range(4):
            resp = httpx.post(url, headers=headers, json=json_payload, timeout=30)
            if resp.status_code == 409 and attempt < 3:
                wait = (attempt + 1) * 3
                _log(f"{label} got 409 (attempt {attempt + 1}/3) — retrying in {wait}s")
                time.sleep(wait)
                continue
            resp.raise_for_status()
            return resp.json()
        raise RuntimeError(f"{label} failed after 3 retries")  # unreachable but satisfies type checker
    ...
    for path, content in files.items():
        ...
        tree_entries.append({"path": path, "mode": "100644", "type": "blob", "sha": data["sha"]})
...
def register(mcp, record_call: Callable) -> None:
    ...
    def generate_rental_website(
        ...
        _log("Waiting 3s for GitHub repo Git objects to initialise")
        time.sleep(3)
        ...
        _push_tree(owner, repo_name, files, f"Initial commit: {site_name} rental website")
