...
def _overlay_signatures_pdf(
    ...
    certificate_page: if True, append a dedicated signature certificate page instead of
                      overlaying on the last page (for uploaded external PDFs)
    """
    from reportlab.pdfgen import canvas as rl_canvas
    from reportlab.lib.utils import ImageReader
    ...
    for page_num in range(1, total + 1):
        ...
        has_final = is_last and not certificate_page and final_sig and final_sig.get("sig_bytes")
    ...
    if certificate_page and final_sig and final_sig.get("sig_bytes"):
        cert_reader = _build_signature_certificate_page(final_sig, school_sig=school_sig)
